...
该接口为```Coinly```向业务方发起回调通知请求，该回调分两种情况，根据回调内容中的status判断，且两种情况是互斥的，只会产生一次回调：
...
发起回调后当接收到返回内容为success字符串时为回调成功，否则为回调失败，回调失败可以进行重新推送，可登录```Coinly```客户端，在项目菜单中选择对应项目，在回调记录菜单中重新推送。
