...
该接口为```Nexa```向业务发发起回调通知请求，该回调分四种情况，根据回调内容中的status判断，且四种情况是互斥的，只会产生一次回调：
| 状态(status) | 描述     |
| :----| :------- |
| 2            | 签名驳回 |
| 4            | 审批驳回 |
...
发起回调后当接收到返回内容为```success```字符串时为回调成功，否则为回调失败，回调失败可以进行重新推送，可登录```Nexa```客户端，在项目菜单中选择对应项目，在回调记录菜单中重新推送。
